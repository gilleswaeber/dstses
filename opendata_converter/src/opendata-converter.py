...
def read_data(dir: str) -> list[tuple]:
	...
	for year in range(min_year, max_year + 1, 1):
		...
		if meteo != None:
			...
			data += [(isoparse(rec[0]), rec[1], rec[2], rec[3], rec[4], float(rec[5]), rec[6]) for rec in
					 list(meteo_records)]
...
for (timestamp, location, param, interval, unit, val, state) in filtered_data:
	if not (timestamp, location) in norm_data_map:
		new_record = (
		datetime(timestamp.year, timestamp.month, timestamp.day, timestamp.hour), location[4:], None, None, None, None,
		None)
...
dataframe.to_csv("dataset_complete_normalized.csv", index=False, na_rep="N/A",
				 header=["Timestamp", "Location", "PM2.5 [µg/m3]", "PM10 [µg/m3]", "Relative Humidity [%]",
...
dataframe.to_csv("dataset_pm10_normalized.csv", index=False, na_rep="N/A",
				 header=["Timestamp", "Location", "PM2.5 [µg/m3]", "PM10 [µg/m3]", "Relative Humidity [%]",
						 "Temperature [°C]", "Pressure [hPa]"])
