...
def moving_average(dataframe: pd.DataFrame, output: bool = True) -> pd.DataFrame:
	...
	dataframe = pd.concat([dataframe[:1], dataframe[:1], dataframe, dataframe[-1:], dataframe[-1:]]).rolling(
		int(window), win_type='exponential', center=True).sum(tau=tau, sym=True).div(f * corr_factor)
