...
async def main():
	...
	with InfluxSensorData(config=config, name="influx") as client:
		...
		forecast_dict = {
			...
			"lstm": forecast_list[0].iloc[:, forecast_list[0].columns.get_loc("Live.PM10_Pred")], # was item 3
			"lstm_seq": forecast_list[1].iloc[:, forecast_list[1].columns.get_loc("Live.PM10_Pred")] # was item 4
		}
	...
	config["influx"]["drops"] = '["pm1", "pm4.0", "pm2.5", "result", "table", "_time", "humidity", "temperature"]'
	with InfluxSensorData(config=config, name="influx") as client:
		# Load the data from the server
