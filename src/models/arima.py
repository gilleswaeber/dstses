...
def train_model_arima(y, x, output: bool = True) -> ARIMA:
	if output:
		logger.info("Training ARIMA model...")
		timer = Timer()
	model = ARIMA(suppress_warnings=True, error_action='ignore')
...
class ArimaModel:
	def __init__(self, conf: ConfigParser, model: ARIMA = None):
	...
	def prepare_model(self, timeseries: pd.DataFrame, output: bool = True) -> ARIMA:
		...
		self.model = train_model_arima(y_train, x_train, output)
	...
	def store(self, config: SectionProxy):
		path = config["storage_location"] + "/ARIMA.pkl"
...
def train_or_load_ARIMA(name: str, config: ConfigParser, data: pd.DataFrame) -> ArimaModel:
	p = config["storage_location"] + "/ARIMA.pkl"
	if os.path.exists(p):
		return load(config)
	else:
