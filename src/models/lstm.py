...
def train_or_load_LSTM(config: SectionProxy, data: DataFrame) -> Tuple[LSTMModel, SectionProxy]:
	path = Path(config["storage_location"]) / f"{config.name}.pkl"
	if path.exists():
...
def to_xy(df: DataFrame, c: LSTMConfig) -> Tuple[np.ndarray, np.ndarray]:
...
def train_lstm_model_predict(config: SectionProxy, df: DataFrame) -> LSTMModel:
