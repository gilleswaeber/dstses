...
class InfluxSensorData(data_adapter.IDataAdapter):
	...
	def __init__(self, config: ConfigParser, name):
		...
		self.drops = json.loads(config[name].get("drops", '["pm1", "pm2.5", "pm4.0", "result", "table"]'))
		self.client = InfluxDBClient.from_config_file(self.config["influx"]["config"], debug=False)

	def get_data(self):
		query = self.client.query_api()

		df: DataFrame = query.query_data_frame('import "date"// Import date library\n'
		...
		df.drop(labels=self.drops, axis=1, inplace=True)
		df.dropna(inplace=True)
		df['_time'] = pandas.to_datetime(df['_time'])
		return df
